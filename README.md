...
2. Створіть файл `.env` на основі `.env.example` та додайте свій ключ OpenAI:
   ```bash
   cp .env.example .env
   ```
...
## Реалізовані AI-функції

Відповідно до вимог тестового завдання, у проєкті реалізовано наступні AI-агенти:
- **Агент пріоритизації (Daily Plan):** Аналізує поточні задачі (статус, пріоритет) та формує обґрунтований план на день (викликається через кнопку "Prioritize").
- **Агент декомпозиції:** Розбиває великі задачі на підзадачі з деталізацією. Агент має вбудовану систему валідації запитів (відхиляє нерелевантні запити).
